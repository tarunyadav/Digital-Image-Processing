## 
 #          File: AreaUsingChainCode.py
 #          Author: Tarun Yadav
 #          Last Modified: April 9, 2012
 #          Topic: Find chain code and calculate area using 4/8 connectivity approach
 #          Instructor: Dr. Anil Seth
 # ----------------------------------------------------------------
 #
 #          Script to find chain codes in a binary image and calculate area of the reagion
...
##Function to find area of all regions using normalized chain codes
#@parameter: array of normalized chain codes for a regions and 4/8 neighbor approach
#@return: area of the given region based on the 4/8 neighbor connectivity
def area_using_chain_code(chain_code,neighbors):
	area=0
	vertical_pos=0
	if(neighbors==4):
		for i in range(0,len(chain_code)):
			if(chain_code[i]=='0'):
				 area = area-vertical_pos
			elif(chain_code[i]=='1'):
...
def region_detection(tolerance,gaussian_smoothing,t_init,neighbors):
	...
	print "Finding Area for all regions using chain codes  "+" ..........."
	area=zeros(region_no+1,dtype=float)
	for i in range(1,region_no+1):
			area[i]=area_using_chain_code(chain_codes[i],neighbors)
	print "Finding Area for all regions using chain codes  "+" ......Done"
	...
	if (gaussian_smoothing==1):
		output_name = '_gaussian_smoothing_'+'_neighbors_'+str(neighbors)+ '_tolerance_'+str(tolerance)
	
	SaveToFile(output_name,t_init,region_no,normalized_chain_codes,area)	
