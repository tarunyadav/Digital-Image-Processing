...
def Dilation(Image, St_element):
	...
	N = st_size[0]
	M=st_size[1]
	Image_dilated= zeros((size[0],size[1]),dtype=float);
	mask = (St_element==1)
...
def Skeleton(Image, St_element):
	size = Image.shape
	skeleton = zeros((size[0],size[1]),dtype=float)
	eroded = Image
	while(any(eroded)==True):
		eroded_open = Erosion(eroded,St_element)
		skeleton[(eroded-Dilation(eroded_open,St_element))!=0]=255
		eroded = eroded_open
	return skeleton
