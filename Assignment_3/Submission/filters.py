## 
 #          File: add_noise.py
 #          Author: Tarun Yadav
 #          Last Modified: Feburary 06, 2012
...
#Function to apply Rotating Average mask filter to image
...
#Function to apply Effcient Median Mask to the image
...
# input for type of filter that user want to add
def user_input_fun():
...
# Calling the user input function for filter type input
filter_type = user_input_fun();		

## Rotating Average Mask case	
if (filter_type==1):
	row_size = int(raw_input("\nType 'height' of mask window[int]: "))
	column_size = int(raw_input("\nType 'width' of mask window[int]: "))	
