...
def boundary_pixels(i_init,j_init,i,j,v,current_region):
	global region_array
	i_old=i
	j_old=j
	if(v==0):
	...
	if ((i==i_init and j==j_init) or i<0 or i>255 or j<0 or j>255):
		return (0,0,5)
		
	if(region_array[i][j]==current_region):
		region_array[i][j]=255
		return (i,j,mod(v+3,4))
	else:
		return (i_old,j_old,mod(v+1,4))
...
def region_detection(tolerance,gaussian_smoothing,t_init):
	global region_array
	global seed_array
	global image_output
	global size
...
def main():		
	...
	region_detection(tolerance,gaussian_smoothing,t_init)
